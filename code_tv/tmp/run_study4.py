...
# random check 
def rand_day_check(start_date, df):

    # Number of days to add
    days_to_add = random.randint(0,9)
...
if tv_count > 0:
    ...
    for each_tv in range(tv_count):
        ...
        if not exclude:
            gaze_df_10d, summary_df = main(args.base_path, ppt_id, start_date, tv_data)
    ...
    rand_day_check(start_date, gaze_df_tvs)
    rand_day_check(start_date, gaze_df_tvs)
    print('###########################################################################')
