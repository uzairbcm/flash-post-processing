...
def preprocess_raw_Chronicle_iPhone_SensorKit_data_files(
    *args,
    Chronicle_iPhone_SensorKit_data_folder: str,
    ignore_list: list,
    ...
    **kwargs,
    ...
    Chronicle_iPhone_SensorKit_data_files = [
        f
        for f in iglob(f"{Chronicle_iPhone_SensorKit_data_folder}/**", recursive=True)
        if os.path.isfile(f)
        and re.search(r"[\s\S]*((iOS|IOS)[\s\S]*Sensor|P[13]-[13]\d{3}|FLASH \d{3})[\s\S]*\.csv", f)
        and all(ignored not in f for ignored in ignore_list)
    ]

    Chronicle_iPhone_SensorKit_data_files.sort()

    for file in Chronicle_iPhone_SensorKit_data_files:
        ...
        try:
            ...
            if specific_date is not None:
                ...
                # print(cat_usels.sum())
                file_name = f"{output_folder}/{participant_id}-A-T1 Chronicle iOS Preprocessed Data for {specific_date} Only.csv"
                new_df.to_csv(file_name, index=False)
                # print("results saved at: ", file_name)
            else:
                # if "P1-" in participant_id and "-A" not in participant_id:
                #     save_name = (
                #         f"{output_folder}/{participant_id}-A-T1 Chronicle iOS Preprocessed Data.csv"
                #     )
                # else:
                #     save_name = (
                #             f"{output_folder}/{participant_id} Chronicle iOS Preprocessed Data.csv"
                #     )
                save_name = (
                    f"{output_folder}/{participant_id} Chronicle iOS Preprocessed Data.csv"
                )
            ...
            print(f"Finished preprocessing Chronicle iPhone SensorKit data for participant {participant_id}")
        except pd.errors.EmptyDataError:
            print(f"The Chronicle iPhone SensorKit data file {file} was empty and will be skipped.")
            continue
        except:
            print(f"An error occurred while preprocessing Chronicle iPhone SensorKit data for participant {participant_id}: {traceback.format_exc()}")
            continue
...
if __name__ == "__main__":
    ...
    OUTPUT_FOLDER = f"{SCRIPT_FOLDER}/Preprocessed Chronicle iPhone SensorKit Data"
    CHRONICLE_IPHONE_SensorKit_DATA_FOLDER = filedialog.askdirectory() # Or specify path manually

    try:
        assert (CHRONICLE_IPHONE_SensorKit_DATA_FOLDER != None) & (CHRONICLE_IPHONE_SensorKit_DATA_FOLDER != "")
    except AssertionError:
        print("Please select the Chronicle iPhone SensorKit data folder using the folder dialog, or specify the path to the folder manually.")
        exit()

    preprocess_raw_Chronicle_iPhone_SensorKit_data_files(
        Chronicle_iPhone_SensorKit_data_folder = CHRONICLE_IPHONE_SensorKit_DATA_FOLDER,
